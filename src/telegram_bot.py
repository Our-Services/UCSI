...
def main():
    ...
    # Run mode: polling (default) or webhook (Cloud Run / serverless)
    use_webhook = str(os.getenv("USE_WEBHOOK", "0")).lower() in ("1", "true", "yes")
    if use_webhook:
        port = int(os.getenv("PORT", "8080"))
        webhook_url = os.getenv("WEBHOOK_URL")
        # Use token as url_path by default (Telegram best practice)
        url_path = os.getenv("WEBHOOK_PATH") or token
        if not webhook_url:
            raise RuntimeError("WEBHOOK_URL must be set when USE_WEBHOOK=1")
        logging.info(f"Starting webhook on 0.0.0.0:{port}, path '{url_path}' → {webhook_url}")
        app.run_webhook(
            listen="0.0.0.0",
            port=port,
            url_path=url_path,
