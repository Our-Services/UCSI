...
def read_cfg() -> Dict[str, Any]:
    # اقرأ الإعدادات مباشرة من JSON بدون الاعتماد على bot.py
    try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Add New User", callback_data="user_add")],
...
def _run_worker(cfg: Dict[str, Any]):
    try:
        set_status("running")
        # استيراد كسول لتفادي فشل الإقلاع عند غياب Playwright
        try:
            from bot import run_bot  # type: ignore
        except Exception as _imp_err:
            set_status("error", "Automation not available on this host (Playwright not installed).")
            return
        run_bot(cfg)
        set_status("done")
...
async def run(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if run_status.get("state") == "running":
        ...
        return
    # تحقّق مبكر إن كانت الأتمتة متاحة على هذا المضيف
    try:
        from bot import run_bot  # noqa: F401
    except Exception:
        await update.message.reply_text("الأتمتة غير متاحة على هذا المضيف (Playwright غير مثبت). شغّلها محليًا أو أنشئ Worker منفصل للأتمتة.")
        return
    cfg = read_cfg()
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "run":
        ...
        try:
            ...
            await q.answer("Automation not available on this host.", show_alert=True)
            await q.message.reply_text("الأتمتة غير متاحة على هذا المضيف (Playwright غير مثبت). شغّلها محليًا أو ضمن Worker منفصل.")
            return
        cfg = read_cfg()
        os.environ["HEADLESS"] = os.getenv("HEADLESS", "1")
        t = threading.Thread(target=_run_worker, args=(cfg,), daemon=True)
...
def main():
    ...
    # Launch output cleaner in the background
...
async def _start_preparation(q, context):
    ...
    if run_status.get("state") == "running":
        ...
        return
    # تأكد أن الأتمتة متاحة قبل البدء
    try:
        from bot import run_bot  # noqa: F401
    except Exception:
        await _replace_message(q, context, "الأتمتة غير متاحة على هذا المضيف (Playwright غير مثبت).", None)
        return
    os.environ["HEADLESS"] = os.getenv("HEADLESS", "1")
    t = threading.Thread(target=_run_worker, args=(cfg,), daemon=True)
    t.start()
